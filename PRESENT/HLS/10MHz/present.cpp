...
void generateRoundKeys80(key_size key_high, block_size key_low) {
	...
	generateRoundKeys80_label1:for (i = 0; i < ROUNDS; i++) {
		key_temp = key_high;
...
block_size sBoxLayer(block_size current_state) {
	block_size output = 0;
	sBoxLayer_label2:for (int i = 0; i < SBOX_SIZE; i++) {
...
block_size PLayer(block_size current_state) {
	block_size output = 0;
	PLayer_label3:for (block_size i = 0; i < PBOX_SIZE; i++) {
...
void present_encrypt(key_size key_high, block_size key_low, block_size block,
	...
	present_encrypt_label4:for (int i = 0; i < ROUNDS - 1; i++) {
		//printf("0x%llX\n", roundKeys[i]);
